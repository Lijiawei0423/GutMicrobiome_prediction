...
class GutFloraDataset(Dataset):
    def __init__(self,data,tmp_lst,target_name):
        ...
        'Set target_data from the target column.'
...
def train_model(model, dataloader, criterion, optimizer, device):
    ...
    for features, targets in dataloader:
        ...
        optimizer.zero_grad()
        'The model output is already passed through sigmoid.'
        ...
        if torch.isnan(outputs).any() or torch.isinf(outputs).any():
            ...
            print(outputs)
            exit()
        # Check whether the current batch contains only one class.
        if (targets > 0).sum() == 0:
            # loss = torch.tensor(0.0, requires_grad=True, device=device)
            loss = (outputs.view(-1) - outputs.view(-1).detach()).pow(2).mean()
        else:
            loss = criterion(outputs.view(-1),(targets>0).float().view(-1))# Treat target > 0 as present, otherwise absent.
        loss = loss.mean()
        loss.backward()
        optimizer.step()
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for features,targets in dataloader:
            features, targets = features.to(device), targets.to(device)
            outputs = model(features.unsqueeze(1))# batch_first=True, so the output layout is (batch, seq, feature)
            targets_binary = (targets > 0).float().to(device)
            # Move tensors from GPU to CPU for metric computation.
            outputs_cpu = outputs.view(-1).detach().cpu()
            targets_binary_cpu = targets_binary.view(-1).cpu()
            targets_binary_cpu = targets_binary_cpu.int()
      
            # Compute AUC only for valid batches that contain at least one positive sample.
            if len(torch.unique(targets_binary_cpu)) > 1:
                # Compute AUC.
                auc = roc_auc_score(targets_binary_cpu, outputs_cpu)
                total_auc += auc
                valid_auc_count += 1
    avg_auc = total_auc/valid_auc_count if valid_auc_count > 0 else 0
    if dist.get_rank() == 0:
        return avg_auc
    else:
        return 0.0    
...
def model_train_valid(train_df,test_df,clade_name,tmp_lst,rank,world_size,device,total_epochs,cv_id):
    ...
    valid_data = GutFloraDataset(test_df, tmp_lst, clade_name)#（16667）
    ...
    for epoch in range(total_epochs):
        train_loader.sampler.set_epoch(epoch) # Keep distributed sampling stable across epochs.
        train_loss = train_model(model,train_loader,criterion,optimizer,device)
        valid_auc =evaluate_model(model,valid_loader,device)
  
        if rank == 0:
            print(f'Epoch {epoch+1}, Train Loss: {train_loss}, Valid AUC: {valid_auc}')
            if epoch == 0:
                best_auc = valid_auc
                ...
                if valid_auc>best_auc:
                    ...
                    best_model_state = model.module.state_dict()
                    model.eval()
                    # Store predictions and targets from the validation split only.
                    all_preds = []
                    all_targets = []
                    all_eids = []
                    with torch.no_grad():
                        for idx, (features, targets) in enumerate(valid_loader):
                            ...
                            all_targets.extend(((targets > 0).int().cpu().numpy()))
    ...
    if rank == 0:
        ...
        model_path = os.path.join(model_dir, f'fold{cv_id}_1_best_model_TotalGain.pth')
        ...
        # risk_path = os.path.join(pred_dir,  f'fold{cv_id}_1_best_riskscore_TotalGain.csv')
        # risk_df.to_csv(risk_path, index=False)
        # print(f'✅ Saved best risk scores to {risk_path}')
        
        print(f'✅After training, Best Validation AUC: {best_auc}')
...
def main(rank,world_size,clade_name):
    ...
    # summary_df = pd.read_csv(dpath+'Analyst_summary.csv')
    # summary_df = summary_df[summary_df['AnalysisType'].isin([1, 2])]
    # xijun_lst = summary_df['Analyst'].tolist() # Full species list used in the experiment.
    ...
    for cv_id in range(5):
        ...
        # pri nt(f"Validation DataFrame size for cv_id {cv_id}: {valid_df.shape}")
    ...
    if rank == 0:
        ...
        # '/home1/LIJW/JiangNan_results2/Abiotrophia_defectiva/S4_Eval'
        save_per_fold_csv = os.path.join('/public/home/LIJW/0129JiangNanResults',clade_name,'S4_Eval/1_fold_results_TotalGain.csv')
