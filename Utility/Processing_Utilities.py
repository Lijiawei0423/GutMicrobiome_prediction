...
def BinaryTo01(binary_var, df):
    ...
    non_null_levels = sorted(my_col.dropna().unique().tolist())
    if my_col.isna().values.any() == False:
        levels = non_null_levels
        my_col_binary = my_col.map({levels[0]: 0, levels[1]: 1})
    else:
        if len(non_null_levels) != 1:
            raise ValueError(f"{binary_var} must contain exactly one non-null level when missing values are present.")
        my_col_binary = my_col.notna().astype(int)
