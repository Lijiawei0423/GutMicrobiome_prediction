...
def _safe_binary_metrics(y_true, y_pred_binary, y_pred_prob):
    ...
    apr = average_precision_score(y_true, y_pred_prob) if len(np.unique(y_true)) > 1 else np.nan
    return acc, sens, spec, prec, Youden, f1, auc, apr
...
def get_avg_score(pred_probs_list, y_test_list, optimal_cutoff):
    ...
    for i in range(nb_folds):
        ...
        evaluations.append(list(_safe_binary_metrics(y_test_list[i], pred_binary, pred_probs_list[i])))
...
def get_full_eval(y_test, pred_prob, cutoff_list):
    ...
    for cutoff in cutoff_list:
        ...
        acc, sens, spec, prec, Youden, f1, auc, apr = _safe_binary_metrics(y_test, pred_binary, pred_prob)
        nnd = np.inf if Youden == 0 else 1/Youden
